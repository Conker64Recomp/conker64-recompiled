...
struct Camera3D {
    ...
    float posX = 0.0f, posY = 2.5f, posZ = -6.0f;
    float rotY = 0.0f;
    float pitch = 18.0f; // Grados de inclinación hacia abajo
    float distance = 6.0f;

    void update(float pX, float pY, float pZ, float camInputX, float dt) {
        // Rotación orbital con stick derecho / teclas
        rotY += camInputX * 120.0f * dt;

        // Suavizado de seguimiento (Lerp)
        targetX += (pX - targetX) * 8.0f * dt;
        targetY += ((pY + 0.8f) - targetY) * 8.0f * dt;
